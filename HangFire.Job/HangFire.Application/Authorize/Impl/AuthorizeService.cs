...
using HangFires.Application.Caching.Authorize;
using HangFire.Common.AccessToken;

namespace HangFires.Application.Authorize.Impl
{
    public class AuthorizeService : HangFireApplicationServiceBase, IAuthorizeService
    {
        ...
        public async Task<ServiceResult<string>> GetAccessTokenAsync(string code)
        {
            ...
            return await _authorizeCacheService.GetAccessTokenAsync(code, async () =>
            {
                ...
                var httpResponse = await client.PostAsync(AccessTokenConfig.API_AccessToken, content);
        ...
        public async Task<ServiceResult<string>> GenerateTokenAsync(string access_token)
        {
            ...
            {
                var url = $"{AccessTokenConfig.API_User}?access_token={access_token}";
                using var client = _httpClient.CreateClient();
                ...
                if (user.Id != AccessTokenConfig.UserId)
                {
